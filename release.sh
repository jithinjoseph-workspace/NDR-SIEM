...
# Usage:
#   ./release.sh 1.1.0          — build both, tag as 1.1.0
#   ./release.sh 1.1.0 engine   — engine only
#   ./release.sh 1.1.0 ui       — UI only
set -euo pipefail
...
printf '%s\n' "$NEW_VERSION" > VERSION
ok "VERSION file and Cargo.toml updated"
...
case "$TARGET" in
    engine) build_engine ;;
    ui)     build_ui ;;
    all)    build_engine; build_ui ;;
    *)      die "Unknown target '$TARGET'. Use: engine | ui | all" ;;
esac

# ── Commit version bump ────────────────────────────────────────────────────────
log "Committing version bump..."
git add VERSION rust/ndr-engine/Cargo.toml
